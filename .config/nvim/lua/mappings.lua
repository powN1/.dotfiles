...
if vim.g.vscode then
  ...
  vim.api.nvim_set_keymap(
    "n",
    "<leader>of",
    ':call VSCodeNotify("remote-wsl.revealInExplorer")<CR> | :call VSCodeNotify("revealFileInOS")<CR>',
    { noremap = true, silent = true, desc = "Open file location in file explorer" }
  )
  ...
  vim.api.nvim_set_keymap(
    "n",
    "<leader>tt",
    ':call VSCodeNotify("workbench.action.terminal.toggleTerminal") <CR>',
    { noremap = true, silent = true }
  )
  ...
  -- function made for mini-animate (for scrolling) to avoid some weird screen flickering when centering the screen after the scroll
  ...
  map("n", "<C-u>", lazy "<C-u>zz", { desc = "Scroll up and center" })
  map("n", "<C-d>", lazy "<C-d>zz", { desc = "Scroll down and center" })
  ...
  map("n", "<leader>6", function() vim.api.nvim_set_current_buf(vim.t.bufs[6]) end, { desc = "Go to Buffer 6" })
  map("n", "<leader>7", function() vim.api.nvim_set_current_buf(vim.t.bufs[7]) end, { desc = "Go to Buffer 7" })
  map("n", "<leader>8", function() vim.api.nvim_set_current_buf(vim.t.bufs[8]) end, { desc = "Go to Buffer 8" })
  map("n", "<leader>9", function() vim.api.nvim_set_current_buf(vim.t.bufs[9]) end, { desc = "Go to Buffer 9" })
